...
class SceneRemoteViewsFactoryGrid(
    ...
    override fun getViewAt(position: Int): RemoteViews {
        ...
        val rv = RemoteViews(context.packageName, R.layout.widget_grid_item)
        ...
        if (colorHex.isNotEmpty()) {
            try {
                ...
                // Usamos un metodo nativo de RemoteViews para colorear el fondo. En API viejas se usaba setInt.
                rv.setInt(R.id.widget_item_container, "setBackgroundColor", colorInt)
            } catch (e: Exception) {}
        } else {
            // Un color por defecto (gris oscuro) si no hay color
